<script>
  ...
  export let noiseIntensity = 0;
  export let scanlineIntensity = 0;
  export let speed = 0.5;
  export let scanlineFrequency = 0;
  export let warpAmount = 0;
  export let disabled = false;
  export let quality = 'auto'; // auto, high, medium, low

  // State stores
  const isMobile = writable(false);
  const isLowPower = writable(false);
  const deviceDPR = writable(1);
  ...
  function detectDevice() {
    ...
    isLowPower.set(isLowPowerDevice);
  }

  // Get resolution scale based on quality setting
  function getResolutionScale() {
    const mobile = $isMobile;
    const lowPower = $isLowPower;
    const q = quality === 'auto' ? (lowPower ? 'low' : mobile ? 'medium' : 'high') : quality;
  ...
  // Initialize WebGL
  function initWebGL() {
    if (!canvas || disabled || initialized) return;
  ...
  function handleResize() {
    ...
    const scaledW = w * $deviceDPR * scale;
    const scaledH = h * $deviceDPR * scale;
...
<div bind:this={container} class="dark-veil-container">
  <canvas bind:this={canvas} class="dark-veil-canvas" />
</div>
